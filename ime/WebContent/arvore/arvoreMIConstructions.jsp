...
<ul <%if (el == null) {%> id="raizMIEdition" <%}%>
	...
	<%
		...
		.getPlayList();
			...
			if (ldep.isAgregado()) {
				if ( !u.hasPermissoes("act.remove.agregado")) countActionLdeps--;
				if ( !u.hasPermissoes("ldep.comp.mi.roles.config")) countActionLdeps--;
			}
			else {
				countActionLdeps = 0;
			}  
	...
	<li id="ldepId-<%=ldep.getId()%>" onmouseover="liHover(event, <%=countActionLdeps%>);" onmouseout="liOut(event);"><a 
		...
		href="app.do?action=ldep.edit&ldep=<%=ldep.getIdentifier()%>">
		
		
		<%=ld.getTitle()%><b style="font-size: 120%;" title="<%=u.getTexto("at.db.ldep.nao.salvo")%>"><%=ldep.flagAlterado && w.constructViewTree==1?"* ":""%></b></a>
 ...
 <%
		...
		<ul <%=ldep.flagTreeView ? "" : "class=\"ulFechado\""%>>
			...
			<a class="arvoreMenos" href="app.do?action=ldep.ftv"> </a><a
				...
				<ul>
					...
					<li onmouseover="liHover(event, <%=countActionRoles%>);" onmouseout="liOut(event);">
 					  ...
 					  <% imeO = (ImeObject) ld.getComponents().getRoles();  %>
            		<%@include file="../../../admin/iconsValidateImsLd.jsp"%>
 						<%
						...
						: "folhaLearners"%>"
								...
								<ul>
									...
									<a class="learner <%=learner.getIdentifier().equals(identifier) ? "selectedEdit" : ""%><%=flagFirst?"":" inherit"%>"
										...
										<span class="learner" style="float: right; padding-right: 5px;">
										<a
											class="controls"
					...
					<li onmouseover="liHover(event, <%=countActionActivities%>);" onmouseout="liOut(event);">
						...
						<a href="activities.do?action=activities.list&ldep=<%=ldep.getIdentifier()%>"><%=u.getTexto("at.cp.at.tit.leg")%></a>
 						...
 						<% imeO = (ImeObject) ld.getComponents().getActivities();  %>
            			<%@include file="../../../admin/iconsValidateImsLd.jsp"%>
 						<%
					...
					<a href="environment.do?action=environment.list&ldep=<%=ldep.getIdentifier()%>"><%=u.getTexto("at.cp.env.tit.leg")%></a>
						...
						<ul>
						    ...
						    %>
							<li class="environment" onmouseover="liHover(event, <%=countActionForEnvs%>);" onmouseout="liOut(event);">
							<a	class="environment-link<%=env.getIdentifier().equals(identifier) ? " selectedEdit"
							: ""%>"
								href="environment.do?action=environment.edit&ldep=<%=ldep.getIdentifier()%>&identifier=<%=env.getIdentifier()%>"><%=env.getTitle()%>
								...
								<ul>
								    <%
		    							...
		    							countActionForEnvs = 1;
		    							if (!u.hasPermissoes("env.lo.remove")) countActionForEnvs--;
								    %>
									...
									: ""%>"
										href="environment.do?action=env.lo.edit&ldep=<%=ldep.getIdentifier()%>&identifier=<%=lo.getIdentifier()%>&id-env=<%=env.getIdentifier()%>"><%=lo.getTitle()%></a>
				   						 ...
				   						 %>
											<span class="learningObjects" style="float: right; padding-right: 5px;"><a
												class="controls"
												href="environment.do?action=env.lo.remove&identifier=<%=lo.getIdentifier()%>&ldep=<%=ldep.getIdentifier()%>&id-env=<%=env.getIdentifier()%>"
												title="<%=u.getTexto("at.cp.env.btn.excluir.lo.titulo")%>"><%=u.getTexto("at.cp.env.btn.lo.excluir.ico")%></a>
											</span>
										<%
											}
										%>		
								...
								<ul>
								    <%
		    							...
		    							countActionForEnvs = 1;
		    							if (!u.hasPermissoes("env.sv.remove")) countActionForEnvs--;
								    %>
									<li  onmouseover="liHover(event, <%=countActionForEnvs%>);" onmouseout="liOut(event);"><a
										class="services <%=sv.getIdentifier().equals(identifier) ? "selectedEdit"
									: ""%>"
										href="environment.do?action=env.sv.edit&ldep=<%=ldep.getIdentifier()%>&identifier=<%=sv.getIdentifier()%>&id-env=<%=env.getIdentifier()%>"><%=sv.getTitle()%></a>
										
													 <% imeO = (ImeObject) sv;  %>
           											 <%@include file="../../../admin/iconsValidateImsLd.jsp"%>
				   						 <%
				   						 	if (u.hasPermissoes("env.sv.remove")) {
				   						 %>
											<span class="services" style="float: right; padding-right: 5px;"><a
												class="controls"
												href="environment.do?action=env.sv.remove&identifier=<%=sv.getIdentifier()%>&ldep=<%=ldep.getIdentifier()%>&id-env=<%=env.getIdentifier()%>"
												title="<%=u.getTexto("at.cp.env.btn.excluir.sv.titulo")%>"><%=u.getTexto("at.cp.env.btn.excluir.sv.ico")%></a>
											</span>
										<%
											}
										%>		
			...
			<a href="play.do?action=play.list&ldep=<%=ldep.getIdentifier()%>"><%=u.getTexto("at.mt.titulo")%> 
						<% imeO = (ImeObject) ld.getMethod();  %>
						<%@include file="../../../admin/iconsValidateImsLd.jsp"%>
			</a>
			<%
							...
							<li class="rp"
								...
								<span class="right" style="padding-right: 5px;">
									...
									href="roles.do?action=roles.edit&ldep=<%=ldep.getIdentifier()%>&identifier=<%=LearningDesignUtils.getIdentifier(ob)%>"><%=ob.getClass().getMethod("getTitle")
										.invoke(ob)%>
											 <% imeO = (ImeObject) ob;  %>
												<%@include file="../../../admin/iconsValidateImsLd.jsp"%>	</a></span>
								<% } %>
										 <a
								class="rp <%=LearningDesignUtils.getIdentifier(rp)
										.equals(identifier) ? "selectedEdit"
										: ""%>"
								href="rp.do?action=rp.edit&identifier=<%=rp.getIdentifier()%>&act-ref=<%=act.getIdentifier()%>&ldep=<%=ldep.getIdentifier()%>"><%=rp.getTitle()%> 
								<% imeO = (ImeObject) rp;  %>
								<%@include file="../../../admin/iconsValidateImsLd.jsp"%>								
								</a>
