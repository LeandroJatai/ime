<%@page import="org.imsglobal.jaxb.ld.Moderator"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	if (sv.getConference() != null)
		mod = sv.getConference().getModerator();
	
	boolean checked = false;
     %>
    <fieldset class="right">
    <legend><%=Lc.getTexto(session, "at.cp.env.sv.conf.mod.leg")%></legend>
    ...
    boolean flagFirst = true;
		...
		<label class="list<%=flagFirst?"":" inherit" %>">
		 ...
		 <input type="radio" name="moderator" value="<%=role.getIdentifier() %>" <%=mod != null && mod.getRoleRef() == role?"checked":""%>/><%=role.getTitle()%></label>
